...
class UPSequentialSimulatorMA(Engine, SequentialSimulatorMixin):
    ...
    def __init__(
        self, problem: "MultiAgentProblem", error_on_failed_checks: bool = True, **kwargs
    ):
    ...
    def _ground_action(
        self, agent: "Agent", action: "up.model.Action", params: Tuple["up.model.FNode", ...]
    ...
    def _is_applicable(
        ...
        try:
            _, reason = self.get_unsatisfied_conditions(
                agent, state, action, parameters, early_termination=True, full_check=True
            )
